...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            // 检查 avatar_url 是否存在且不为空，且不是字符串 "undefined"
            if (this.avatar_url && this.avatar_url !== "undefined" && this.avatar_url.indexOf('/') === -1) {
        ...
        methods:{
            ...
            changeInfo(){
                 ...
                 let r = ElMessageBox.confirm('确认修改个人信息吗?',{
                     ...
                     // 处理 avatar_url
                     let avatarPart = "";
                     if (that.avatar_url && typeof that.avatar_url === 'string') {
                         let parts = that.avatar_url.split('/');
                         if (parts.length > 3) {
                            avatarPart = parts[3];
                         }
                     }
